...
9. **Frontend challenge workspaces branch on `challenge.kind`:** Monaco Editor (PROGRAMMING), a MathLive `<math-field>` visual editor + on-screen math keyboard that emits LaTeX (MATH), and a single/multi-select quiz (MCQ → `POST /mcq/submit`). Multi-challenge lessons render a `ChallengeTabs` selector; the onboarding wizard lives at `/onboarding`.
10. **Frontend uses Tailwind CSS** with a **clean, technical** design language (Linear/Vercel-style): flat, hairline-bordered surfaces, one restrained indigo accent, and **no gradient/glass/glow** decoration. Shared utilities `.glass-panel`/`.glass-card` are now flat (names kept for compatibility), `.text-gradient` is a solid accent, plus `.icon-tile`/`.eyebrow`. See `Frontend/DESIGN_SYSTEM.md`.
